...
function inicializarAlertas(idMascota) {
  ...
  function construirHtmlAlerta(alerta) {
    ...
    return "<div class=\"bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow flex items-start gap-4 cursor-pointer " + opacidad + "\" onclick=\"marcarAlertaComoLeida(" + alerta.id + ")\">" +
  ...
  window.marcarAlertaComoLeida = function(idAlerta) {
    fetch("/spring/analisis/alertas/" + idAlerta + "/leer", {
      ...
      .catch(function(error) { console.error("Error marcando alerta como leida:", error); });
  };
