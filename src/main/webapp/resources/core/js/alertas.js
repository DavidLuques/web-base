...
function inicializarAlertas(idMascota) {
  ...
  function construirHtmlAlerta(alerta) {
    ...
    const badge = alerta.leido
      ? `<span class="bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full font-bold">&#10003; Leido</span>`
  ...
  function cargarAlertasMascota() {
    fetch("/spring/analisis/alertas/datos/" + idMascota)
      .then(response => {
        if (!response.ok) throw new Error("Error en el servidor. Codigo HTTP: " + response.status);
        ...
        if (!listaDeAlertas || !Array.isArray(listaDeAlertas) || listaDeAlertas.length === 0) {
          contenedor.innerHTML = `
            <div class="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-sm">
              ...
              <p class="text-sm text-slate-500">No tenes notificaciones de amigos ni transferencias.</p>
            </div>
          `;
          lucide.createIcons();
          return;
  ...
  window.marcarAlertaComoLeida = function(idAlerta) {
    fetch(`/spring/analisis/alertas/${idAlerta}/leer`, {
      ...
      .catch(error => console.error("Error marcando alerta como leida:", error));
  };

  // Solo cargar alertas de mascota si hay una mascota seleccionada
